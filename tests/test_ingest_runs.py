...
def test_valid_line_parses():
    row = parse_run_line(make_line(), "runs.jsonl", 1)
    ...
    assert row[9] is None  # no extra


def test_unknown_fields_go_to_extra():
    row = parse_run_line(make_line(host="nas"), "runs.jsonl", 1)
    assert json.loads(row[9]) == {"host": "nas"}


def test_missing_run_id_skips():
    line = make_line()
    obj = json.loads(line); del obj["run_id"]
    assert parse_run_line(json.dumps(obj), "runs.jsonl", 1) is None


def test_corrupt_and_blank_lines_skip():
    assert parse_run_line("{nope", "runs.jsonl", 2) is None
    assert parse_run_line("   ", "runs.jsonl", 3) is None
