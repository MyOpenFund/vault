...
def test_new_indexes_exist_and_train_is_idempotent(clean_db, tmp_path, monkeypatch):
    ...
    assert EXPECTED_INDEXES <= set(defs)
    # Names alone would pass on a plain b-tree over the same columns; the two
    # indexes whose whole point is their shape are checked on their definition.
    assert "WHERE (deleted_at IS NULL)" in defs["idx_documents_live_agg"]
    assert "jsonb_path_ops" in defs["idx_documents_extra_gin"]
